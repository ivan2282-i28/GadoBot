...
from cards_service import (
    ...
    add_user_card,
    cards,
    check_and_reroll_duplicate_cards,
    get_user_unique_cards,
    remove_card_from_system,
    send_card_with_image,
    show_card_page,
    user_pagination,
)
...
def register_handlers(bot: Bot, dp: Dispatcher, api_token: str) -> None:
    ...
    async def cmd_roll_card(message: Message) -> None:
    ...
    async def cmd_see_cards(message: Message) -> None:
        user_id = message.from_user.id
        user_cards = await get_user_unique_cards(user_id)

        if not user_cards:
            await message.answer(
                "You don't have any cards yet! Use /rc to roll your first card."
            )
            return
        ...
        await show_card_page(bot, message, user_id, 0)
